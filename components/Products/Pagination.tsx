...
const Pagination = ({ numberOfProducts, currentPage }: PropTypes) => {
    ...
    pageItems.push(
      <button
        ...
        className={`btn-brutalist relative inline-flex items-center justify-center transition-all duration-300 ${
          isSelected 
            ? 'bg-black text-white' 
            : 'bg-white text-current hover:bg-primary hover:text-current'
        }`}
  ...
  return (
    <div className="w-full border-t border-none/5 bg-white py-12">
      <div className="max-w-[1920px] mx-auto px-4 md:px-12 flex flex-col items-center gap-8">
        ...
        <div className="text-[10px]  uppercase tracking-[0.3em] text-gray-400">
          Index <span className="text-current">{firstProductIndex} — {lastProductIndex}</span> 
          <span className="mx-4 text-gray-200">/</span> 
          Total <span className="text-current">{numberOfProducts}</span>
        </div>

        {/* NAVIGATION AREA - Zadržana brutalistička crna senka */}
        <nav className="inline-flex border-1 border-none overflow-hidden bg-white">
          {/* PREVIOUS */}
          <button
            type="button"
            ...
            className={`btn-brutalist flex items-center justify-center transition-colors ${
              currentPage === 1 ? 'opacity-20 cursor-not-allowed' : 'hover:bg-primary hover:text-current'
            }`}
          >
          ...
          <button
            ...
            onClick={() => exportQueryParameters('page', (currentPage + 1).toString())}
            className={`btn-brutalist flex items-center justify-center transition-colors ${
              lastProductIndex === numberOfProducts ? 'opacity-20 cursor-not-allowed' : 'hover:bg-primary hover:text-current'
            }`}
          >
